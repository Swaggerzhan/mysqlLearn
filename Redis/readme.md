...
字典即哈希表，使用某些算法将输入映射到一个固定值，在通过一些特定的运算找到对应数组中的索引，进而得到O(1)级别的查找效率。

对于哈希冲突可以使用许多种方法进行处理，这里我们假设此哈希表长度为10。

* 线性探测法

如果 key % 10 = 4，切 index为4的位置已经有元素了，则使用 (key + 1) % 10的策略进行计算，以此类推，查找时也是如此。

* 平方探测法 和 随机探测法

和线性探测法类似，只是遇到key冲突的增量选择不同，平方探测法采用 + 1^2 - 1^2 + 2^2 - 2^2 ...的方式进行计算，而随机探测法则提前计算了好了随机的数据，如 2，5，9....，遇到冲突的计算方式则是(key + 2) % 10，如果还是冲突，则 (key + 5) % 10，以此类推。
